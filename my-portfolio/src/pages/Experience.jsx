...
const Experience = () => {
	...
	return (
		!isLoading &&
		data.map((entry) => (
			<div className='flex flex-col mx-auto sm:w-4/5 md:w-3/4 lg:w-2/3 p-8 mb-8 border-2 border-cyan-400 rounded-xl'>
				<div className='relative flex flex-wrap-reverse pb-4 mb-4 border-b-2 border-cyan-400'>
					<div className='flex flex-col'>
						...
						<p className='text-sm md:text-base lg:text-lg text-white font-light font-sans'>
							{ToMMYYYYFormat(`${entry.startDate}`)}
							{" - "}
							{ToMMYYYYFormat(`${entry.endDate}`)}
						</p>
					</div>
					<div className='flex flex-1 flex-shrink-0 items-center px-4 justify-center'>
						<img
							src={urlFor(entry.image).quality(100).url()}
							alt='Company Logo'
							className='max-w-24 md:max-w-28 lg:max-w-32 block'
						/>
					</div>
				</div>
				<CustomPortableText value={entry.description} />
			</div>
		))
	);
};
